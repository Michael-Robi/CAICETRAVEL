<?php
	...
	// consulta para mostrar las reservas
	$idResevaH=$_GET['idResevaH'];
	$sql="SELECT r.consecutivo consecutivo, r.hora hora, r.fechaIngreso fechaIngreso, r.fechaSalida fechaSalida, u.nombre usuario, h.descripcion habitacion, h.precio precio FROM usuarios u, reserva r, reservahotel rh, habitacion h WHERE u.idUsuarios=r.usuario AND r.consecutivo=rh.reserva AND rh.habitacion=h.codigo AND r.consecutivo='$idResevaH'";
    ...
    if(isset($_POST['btnGrabar']))
	{
		...
		$habitacionC = strip_tags(trim($_POST['habitacionC']));
		...
		if($consecutivo=="")	{
			...
			try {
				...
				if($nombreU==""){
					...
					// 	consultar diferencia en dias
					$stmt = $usuario->runQuery("SELECT DATEDIFF('$fechaSalida','$fechaIngreso') diasReservados");
					$stmt->execute();
					$row=$stmt->fetch(PDO::FETCH_ASSOC);

					$diasReservados=$row['diasReservados']+1;

					// consultar valor de la habitacion
					$stmt = $usuario->runQuery("SELECT precio FROM habitacion WHERE codigo='$habitacionC'");
					$stmt->execute();
					$row=$stmt->fetch(PDO::FETCH_ASSOC);

					$precioHabitacion=$row['precio'];

					$TotalAPagar=number_format($diasReservados*$precioHabitacion,3);

					// modificar cantidad habitacion
					// habitacion anterior

					$Habitacion=$fila['habitacion'];
					$stmt = $usuario->runQuery("SELECT cantidad, codigo FROM habitacion WHERE descripcion='$Habitacion'");
					$stmt->execute();
					$row=$stmt->fetch(PDO::FETCH_ASSOC);
					$cantidad=$row['cantidad']+1;
					$codigoHabitacion=$row['codigo'];

					$sql="UPDATE reserva SET hora='$hora', fechaIngreso='$fechaIngreso', fechaSalida='$fechaSalida', usuario='$nombreU' WHERE consecutivo='$consecutivo'";

					$stmt=$usuario->runQuery($sql);
					$stmt->execute();

					$sql1="UPDATE reservaHotel SET habitacion='$habitacionC', precio='$TotalAPagar' WHERE reserva='$consecutivo'";
					
					$stmt=$usuario->runQuery($sql1);
					$stmt->execute();

					// echo "M -> CS $sql"."<br>"."M-> H-1 $sql1";

					if ($codigoHabitacion==$habitacionC) {
						...
						$sql2="UPDATE habitacion SET cantidad='$cantidad' WHERE codigo='$codigoHabitacion'";

						$stmt = $usuario->runQuery($sql2);
						$stmt->execute();

						//habitacion nueva
						$stmt = $usuario->runQuery("SELECT cantidad nuevaC FROM habitacion WHERE codigo='$habitacionC'");
						$stmt->execute();
						$row=$stmt->fetch(PDO::FETCH_ASSOC);
						$cantidadN=$row['nuevaC']-1;

						$sql3="UPDATE habitacion SET cantidad='$cantidadN' WHERE codigo='$habitacionC'";

						$stmt = $usuario->runQuery($sql3);
						$stmt->execute();

						// echo "<br> A-> H+1 $sql2"."<br>"."N-> H-1 $sql3";
					}
...
<head>
	...
	<title>Actualizar Reservar Hotel</title>
	...
	<form class="col s12 m12 l12" name="enviar" method="POST" enctype="multipart/form-data">
		...
		<div class="col">
			...
			<div class="input-field col s12 m8 l6 offset-l1 offset-m1">
				...
				placeholder=" Ingrese el consecutivo" class="color" value="<?php echo $fila['consecutivo']; ?>" readonly>
	        ...
	        <div class="input-field col s12 m8 l6 offset-l1 offset-m1">
    			...
    			<input type="text" id="hora" name="hora" placeholder=" Ingrese la hora de la reserva" class="timepicker color"  value="<?php echo $fila['hora']; ?>" readonly>
			...
			<div class="input-field col s10 m8 l6 offset-l1 offset-m1">
				...
				<input type="text" id="fechaIngreso" name="fechaIngreso" placeholder=" Ingrese la fecha de Ingreso" class="datepicker color" value="<?php echo $fila['fechaIngreso']; ?>" readonly>
			...
			<div class="input-field col s10 m8 l6 offset-l1 offset-m1">
				...
				<input type="text" id="fechaSalida" name="fechaSalida" placeholder=" Ingrese la fecha de salida" class="datepicker color" value="<?php echo $fila['fechaSalida']; ?>" readonly>
            ...
            <div class="input-field col s12 m8 l6 offset-l1 offset-m1">
				...
				<input type="text" id="nombre" name="nombre" placeholder=" Ingrese usuario o email" class="color" value="<?php echo $fila['usuario']; ?>" readonly>
	          	...
	          	<?php
					...
					$stmt->execute();
      				...
      				while ($fila1=$stmt->fetch(PDO::FETCH_ASSOC)) {
      					...
      					$codigoHabitacion=$fila1['codigo'];
      					$Habitacion=$fila1['descripcion'];
      					$Precio=$fila1['precio'];

      					if($descripcionH==$Habitacion){
							echo '<option value="'.$codigoHabitacion.'"
  			      			selected>'.$Habitacion.' $'.$Precio.'</option>';
      					}

      					else{
      						echo '<option value="'.$codigoHabitacion.'">'.$Habitacion.' $'.$Precio.'</option>';
      					}
			        }
			      ?>
	          </select>
			...
			<!-- Mostrar el Input, con la habitación escogida-->
  			<div class="input-field col s12 m8 l6 offset-l1 offset-m1">
				<i class="material-icons prefix">airline_seat_individual_suite</i>
				<input type="text" id="habitacionE" name="habitacionE" placeholder=" Habitación Escogida" class="color" value="<?php echo $fila['habitacion']; echo ' $'.$fila['precio']; ?>" readonly>
			...
			<!-- Mostrar el Input, con la habitación escogida-->
  			<!-- <div class="input-field col s12 m8 l6 offset-l1 offset-m1"> -->
				<!-- <i class="material-icons prefix">airline_seat_individual_suite</i> -->
				<input type="hidden" id="habitacionC" name="habitacionC" placeholder=" Habitación Escogida" class="color" value="
				<?php 
					$descripcionH=$fila['habitacion']; 
					$sql="SELECT * FROM habitacion where hotel='$codigo' AND cantidad<>'0'";
				    $stmt=$usuario->runQuery($sql);
