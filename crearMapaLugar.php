<?php
  ...
  class cargarBD{
    ...
    function crearXMLHotel(){
      ...
      $sql1 = "SELECT codigo, descripcion, detalle, ubicacion, TO_BASE64(imagen) as imagen, latitud, longitud, tipo FROM lugar";
      ...
      foreach ($this->conn->query($sql1) as $row)
      {
        ...
        echo '<marker ';
        echo 'codigo_lugar="'.parseToXML($row['codigo']).'" ';
        echo 'descripcion_lugar="'.parseToXML($row['descripcion']).'" ';
        echo 'detalle_lugar="'.parseToXML($row['detalle']).'" ';
        echo 'ubicacion_lugar="'.parseToXML($row['ubicacion']).'" ';
        echo 'imagen_lugar="'.$row['imagen'].'" ';
        echo 'latitud_lugar="'.$row['latitud'].'" ';
        echo 'longitud_lugar="'.$row['longitud'].'" ';
        echo 'tipo_lugar="'.$row['tipo'].'" ';
        echo '/>';
      }
