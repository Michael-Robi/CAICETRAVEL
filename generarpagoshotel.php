<?php
	...
	$sql1="SELECT u.nombre usuario, h.descripcion habitacion, DATEDIFF(r.fechaSalida,r.fechaIngreso)+1 diasReservados, rh.precio totalAPagar, DATE_FORMAT(r.hora, '%h:%i %p') horaIngreso, r.fechaIngreso fechaIngreso, r.fechaSalida fechaSalida FROM usuarios u, reserva r, reservaHotel rh, habitacion h, hotel ho WHERE u.idUsuarios=r.usuario AND r.consecutivo=rh.reserva AND rh.habitacion=h.codigo AND h.hotel=ho.codigo AND ho.codigo=$codigo ORDER BY usuario,habitacion ASC";

	$stmt1 = $administrador->runQuery($sql1);
    $stmt1->execute();

    $sql2="SELECT h.descripcion hotel, SUM(DATEDIFF(r.fechaSalida,r.fechaIngreso)+1) diasReservados, SUM(rh.precio) totalAPagar FROM reserva r, reservahotel rh, habitacion ha, hotel h WHERE r.consecutivo=rh.reserva AND rh.habitacion=ha.codigo AND ha.hotel=h.codigo AND h.codigo=$codigo GROUP BY h.descripcion ORDER BY hotel ASC";
    $stmt2 = $administrador->runQuery($sql2);
    $stmt2->execute();

	$sql="SELECT h.descripcion hotel, ha.descripcion habitacion, ha.precio precio, SUM(DATEDIFF(r.fechaSalida,r.fechaIngreso)+1) diasReservados, SUM(rh.precio) totalAPagar FROM reserva r, reservahotel rh, habitacion ha, hotel h WHERE r.consecutivo=rh.reserva AND rh.habitacion=ha.codigo AND ha.hotel=h.codigo AND h.codigo=$codigo GROUP BY rh.habitacion ORDER BY hotel ASC";
