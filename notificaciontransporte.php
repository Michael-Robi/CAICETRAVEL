<html lang="es">
	...
	<body>
		<?php
			...
			$asunt="Recordatorio Reserva Transporte";

			$sql1 ="SET lc_time_names ='es_ES'";
			$stmt = $administrador->runQuery($sql1);
	    	$stmt->execute();

			$sql="SELECT rt.reserva idReserva, u.nombre nombre, u.email email, t.descripcion transporte, t.ubicacion direccion, DATE_FORMAT(r.fechaIngreso, '%W %d ' 'de' ' %M ' 'del' ' %Y') fecha, DATE_FORMAT(r.hora, '%h:%i %p') hora FROM usuarios u, reserva r, reservatransporte rt, transporte t WHERE u.idUsuarios=r.usuario AND rt.reserva=r.consecutivo AND rt.transporte=t.codigo AND r.fechaIngreso BETWEEN CURDATE() AND '2018-12-31'";
